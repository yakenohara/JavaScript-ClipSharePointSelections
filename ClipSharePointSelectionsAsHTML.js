...
(async () => {
  ...
  strarr_mkdnLinks.push(`<a href="${str_CurDirNameEncoded}">${str_CurDirNameDecoded}<a>`);
  ...
  /* Markdown 文字列の作成 */
  if (0 < strarr_selectedItemNames.length){
    strarr_mkdnLinks.push(`<ul>`);
    for (let int_selectIdx = 0 ; int_selectIdx < strarr_selectedItemNames.length ; int_selectIdx++){ /* 配列要素最後 - 1 までの間は、行頭を`┣`で開始する */
      let str_selectionName = strarr_selectedItemNames[int_selectIdx];
      let str_mkdnLink = strarr_mkdnLinkOfSelections[int_selectIdx];
      strarr_mkdnLinks.push(`    <li><a href="${str_mkdnLink}">${str_selectionName}</a>`);
    }
    strarr_mkdnLinks.push(`</ul>`);
  }
